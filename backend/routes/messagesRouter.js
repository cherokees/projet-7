...
import { addMessage, getAllMessages, putMessageById, getMessageById, disableMessageById } from '../control/messages';
...
router.put("/disable/:id", auth, authUserId, async (req, res, next) => {
    try {
        const rows = await disableMessageById(req.body.postId);
        const refreshMessage = await getMessageById(req.body.postId);
        res.status(200).json({ data: refreshMessage, message: "le rafraichissement c'est fait" });
    } catch (err) {
        console.error(err); // On affiche l'erreur côté serveur
